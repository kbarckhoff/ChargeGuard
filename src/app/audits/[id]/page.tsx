  ...
  return (
    <>
      <header className="h-14 border-b border-[#e2e8f0] bg-white px-6 flex items-center justify-between flex-shrink-0">
        <div>
          <h1 className="text-base font-semibold text-[#0f172a]">{audit.hospital_name}</h1>
          <p className="text-xs text-[#94a3b8]">{audit.name}</p>
        </div>
                  ...
                  <div key={phase.id} className="flex items-start">
                    <Link
                      ...
                      className={`flex flex-col items-center min-w-[100px] px-2 py-2 rounded-lg transition-all ${
                        isActive ? "bg-[#0f172a] text-white" : "hover:bg-[#f1f5f9]"
                      }`}
                    >
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-sm mb-1.5 ${
                          isDone
                            ...
                            : isActive ? "bg-[#334155] text-[#94a3b8]" : "bg-[#eef2f6] text-[#64748b]"
                        }`}
                      >
                        {isDone ? <Check size={14} /> : phase.phase_number}
                      </div>
                      <span className={`text-[11px] font-medium text-center leading-tight ${isActive ? "text-white" : "text-[#334155]"}`}>
                    ...
                    {i < (phases?.length || 0) - 1 && (
                      <div className="flex items-center pt-4 px-0">
                        <div className={`w-4 h-0.5 ${isDone ? "bg-emerald-300" : "bg-[#e2e8f0]"}`} />
                      </div>
                    )}
                  </div>
                );
