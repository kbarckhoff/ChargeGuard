# ChargeGuard Engine Build — an expert consultant Methodology (v1)

Built the core analysis engine by porting the expert's CDM review logic into the scan pipeline.

## What was added

**CMS reference layer** (Track A foundation)
- `data/cms_reference.csv` — 2,789 unique HCPCS codes with SI, APC payment, MC/PF fee schedule, CLFS, ASP, and retired flag. Starter set extracted from the expert's v9-3 Report ("Hospital CDM + All Flags" tab).
...
- `src/lib/cdm-reference-rules.ts` — `runReferenceRules()` implementing:
  ...
  - Rev Code 637 — Self-Administered Drugs, Item 10 (`637`) — 700 lines, matches the expert's report exactly
...
## Verification against the expert's v9-3 Report (sample CDM, 8,080 lines)

| Flag | Engine | the expert's report | Result |
|------|--------|---------------|--------|
...
## Notes / next steps
- The bilateral 1.75× *ratio* check (`10`) needs a real CDM import with separate RT/LT/50 lines and multiple modifier columns to fully engage; the single-modifier sample under-exercises it (the missing-RT/LT detection already matches the reference methodology at 51 vs 52).
