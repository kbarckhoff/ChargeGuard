# ChargeGuard — Porting the expert's CDM Methodology into the Analysis Engine

Goal: make ChargeGuard reproduce an expert consultant's CDM review on any client's chargemaster.
Source of truth: `the source CDM files\CDM_Formula_Library.xlsx` (24 steps), `CDM_Analysis_Replication_Guide.docx`, `CDM_Analysis_Report_v9-3.xlsx` (32 tabs / 26 flags), `CDM_Review_Dictionary_Reference.xlsx`.

## How the engine works today
`src/app/api/scan/scan-route-v2.ts` runs self-contained rules over imported `ChargeItem` rows and writes `Finding` rows. It already uses the expert's rule numbering. Current coverage:

| Rule | What it checks | the reference methodology step |
|------|----------------|-----------|
...
## What the expert's method adds (the gaps)
the expert's highest-dollar flags are **status-indicator (SI) driven** and depend on CMS reference data joined onto each HCPCS. From the Formula Library + Report:

| the reference methodology step / flag | What it does | Sample-client $ | Needs reference data |
|---|---|---|---|
...
Recommendation: **Track A** — it's what actually makes ChargeGuard "reproduce the expert's review." Track B rules can be layered in alongside.

## Open question for Track A
Need the CMS reference files the expert's formulas point to (CPT Addendum B, OPPS Addendum A, MPFS, CLFS, ASP, NDC/C-code crosswalk). Either locate them on disk, or extract the SI/APC columns already joined into the v9-3 Report as a starter reference set.
